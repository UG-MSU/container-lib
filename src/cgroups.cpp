...
void init_cgroup(uint64_t MEM_SIZE, double TOTAL_CPU_PERCENTAGE,
    ...
    case 2: { // cgroup v2
        ...
        echo_to_file(cgroup + "/cpu.max",
                     intstr((uint64_t)(1000000 * TOTAL_CPU_PERCENTAGE)) + " " +
                         intstr(1000000));              // set max cpu time);
        echo_to_file(cgroup + "/memory.swap.max", "0"); //  disable memory swap
        echo_to_file(cgroup + "/memory.max",
                     intstr(MEM_SIZE * 1024)); // set max memory
        break;
    }
    case 1: { // cgroup v1
        ...
        echo_to_file(cgroup + "/cpuset.cpus",
                     intstr(CPU)); // set random cpu core
        echo_to_file((cgroup + "/cpu.max").c_str(),
                     intstr((uint64_t)(1000000 * TOTAL_CPU_PERCENTAGE)) + " " +
                         intstr(1000000)); // set max cpu time
        mkdir((CGROUP_PATH + "/memory/yats").c_str(), 0700);
        mkdir((CGROUP_PATH + "/memory/yats/" + CGROUP_ID).c_str(), 0700);
        echo_to_file((cgroup + "/memory.swap.max").c_str(),
                     "0"); //  disable memory swap
        echo_to_file((cgroup + "/memory.max").c_str(),
                     intstr(MEM_SIZE * 1024)); // set max memory
        break;
    }
...
void add_to_cgroup(pid_t pid, std::string CGROUP_ID) {
    ...
    switch (_cversion) {
    case 2: {
        chmod((MAIN_CGROUP_PATH + "/" + CGROUP_ID + "/cgroup.procs").c_str(),
              0777);
        echo_to_file(MAIN_CGROUP_PATH + "/" + CGROUP_ID + "/cgroup.procs",
                     intstr(pid));
        break;
    }
    case 1: {
        echo_to_file(CGROUP_PATH + "/cpuset/yats/" + CGROUP_ID + "/tasks",
                     intstr(pid));
        echo_to_file(CGROUP_PATH + "/cpu/yats/" + CGROUP_ID + "/tasks",
                     intstr(pid));
        echo_to_file(CGROUP_PATH + "/memory/yats/" + CGROUP_ID + "/tasks",
                     intstr(pid));
        break;
    }
    case 0: // no cgroup mounted (error)
