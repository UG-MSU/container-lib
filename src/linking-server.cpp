...
void ContainerLib::ContainerDocker::client::start(
    std::vector<std::string> args, std::string path2file,
    std::string path2input,
    ContainerLib::Container::launch_options launch_options) {
...
size_t ContainerLib::ContainerDocker::client::read_complete(
    char *buf, const error_code &err, size_t bytes) {
    if (err)
        return 0;
...
ContainerLib::ContainerDocker::client::client(int port, std::string local_add) {
    ip::tcp::endpoint ep(ip::address::from_string(local_add), port);
    this->ep = ep;
}

void ContainerLib::ContainerDocker::client::sync_echo(std::string msg) {
...
std::string ContainerLib::ContainerDocker::client::copy(std::string path2file) {
...
std::string
ContainerLib::ContainerDocker::client::vec2str(std::vector<std::string> vec) {
...
std::string
ContainerLib::ContainerDocker::client::read_from_buff(ip::tcp::socket &sock) {
    int bytes;
    char buff[65536];
...
std::string
ContainerLib::ContainerDocker::client::file2string(std::string path2file) {
