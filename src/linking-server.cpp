...
void client::start(std::vector<std::string> args, std::string path2file, std::string path2input) {
...
client::client(int port, std::string local_add) {
  ip::tcp::endpoint ep(ip::address::from_string(local_add), port);
  this->ep = ep;
}

void client::sync_echo(std::string msg) {
...
std::string client::copy(std::string path2file) {
...
std::string client::vec2str(std::vector<std::string> vec) {
...
std::string client::read_from_buff(ip::tcp::socket & sock) {
  int bytes;
	char buff[65536];
...
std::string client::file2string(std::string path2file) {
